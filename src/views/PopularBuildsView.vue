...
// Builds resolved synchronously so the cards are pre-rendered during SSG
const builds = computed<PopularBuild[]>(() =>
  injectedBuilds.length > 0 ? injectedBuilds : (props.popularBuilds || [])
)

// Per-route SEO, baked into the static HTML via useHead
const title = 'Popular Builds - Expedition 33 Builds'
...
useHead({
  ...
  link: [
    { rel: 'canonical', href: 'https://www.expedition33builds.com/popular/' },
  ],
})
